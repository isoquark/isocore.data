...
module ``CsvReader Test`` =
    ...
    let private hydrate<'T>(resname) =
        let text = thisAssemblyElement() |> ClrAssembly.findTextResource resname |> Option.get
    ...
    let ``Desribed CSV file - no attribute overrides``() =
        ...
        let path = thisAssemblyElement() |> ClrAssembly.writeTextResource resname (TestContext.getTempDir())
        let format = CsvReader.getDefaultFormat()

...
module DateGeneration =
    ...
    type DateGenerator(range: Range<BclDateTime>) =
        ...
        let yGen = NumberGenerator.withRange<int32>(range.MinValue.Year, range.MaxValue.Year)
        let mGen = NumberGenerator.withRange<int32>(range.MinValue.Month, range.MaxValue.Month)
       
        let getNextValues(count, range : Range<BclDateTime>) =            
            [|for i in 1..count do
                let y = yGen.NextValue()
                let m = mGen.NextValue()
                let dGen = NumberGenerator.withRange<int32> (1, Calendar.GetDaysInMonth(y, m))
